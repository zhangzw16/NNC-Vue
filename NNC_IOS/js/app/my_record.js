...
//					breakfast_info_submit.window.clearBox(bf);
		...
		h('#share').tap(function() {
			...
			var bdScheme = NSURL.URLWithString("weixin://");
			var bdScheme1 = NSURL.URLWithString("mqq://");
			var bdScheme2 = NSURL.URLWithString("sinaweibo://");
			var install = app.canOpenURL(bdScheme);
			var install1 = app.canOpenURL(bdScheme1);
			var install2 = app.canOpenURL(bdScheme2);
			
			plus.ios.deleteObject(bdScheme);
			plus.ios.deleteObject(bdScheme1);
			plus.ios.deleteObject(bdScheme2);
			...
			if(!install){
				var num,num2 ;
				$.each(actionbuttons,function(i,n){
					if(n.title == '微信好友'){
						num = i;
					}
				})
				actionbuttons.splice(num,1);
				$.each(actionbuttons,function(i,n){
					if(n.title == '朋友圈'){
						num2 = i;
					}
				})
				actionbuttons.splice(num2,1);
			}
			if(!install1){
				var num ;
				$.each(actionbuttons,function(i,n){
